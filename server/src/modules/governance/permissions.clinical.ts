import { RoleKey, type WorkstreamPermissions } from './roles.js';

/**
 * CLINICAL CORE permissions — owned by Agent 2.
 *
 * Add clinical permissions here (patient, encounter, intake, vitals, notes,
 * timeline, prescription, reports) and grant them to existing roles via
 * `roleGrants`. ADMIN automatically receives every permission, so never list
 * ADMIN here. Do not edit other workstreams' permission files.
 */
export const ClinicalPermission = {
  PATIENT_REGISTER: 'patient:register',
  PATIENT_READ: 'patient:read',
  PATIENT_SEARCH: 'patient:search',
  PATIENT_UPDATE: 'patient:update',
  PATIENT_MERGE: 'patient:merge',
  PATIENT_CONTACT_READ: 'patient:contact:read',
  PATIENT_CONTACT_WRITE: 'patient:contact:write',
  PATIENT_IDENTIFIER_READ: 'patient:identifier:read',
  PATIENT_IDENTIFIER_WRITE: 'patient:identifier:write',
  QR_ISSUE: 'qr:issue',
  QR_RESOLVE: 'qr:resolve',
  ENCOUNTER_CHECKIN: 'encounter:checkin',
  ENCOUNTER_READ: 'encounter:read',
  ENCOUNTER_STATUS: 'encounter:status',
  QUEUE_READ: 'queue:read',
  APPOINTMENT_READ: 'appointment:read',
  APPOINTMENT_SCHEDULE: 'appointment:schedule',
  APPOINTMENT_UPDATE: 'appointment:update',
  APPOINTMENT_CANCEL: 'appointment:cancel',
  APPOINTMENT_ARRIVAL: 'appointment:arrival',
  APPOINTMENT_OVERBOOK: 'appointment:overbook',
  SCHEDULE_CONFIG_READ: 'schedule:config:read',
  SCHEDULE_CONFIG_MANAGE: 'schedule:config:manage',
  INTAKE_RECORD: 'intake:record',
  INTAKE_READ: 'intake:read',
  VITALS_RECORD: 'vitals:record',
  VITALS_READ: 'vitals:read',
  OBSERVATION_RECORD: 'observation:record',
  OBSERVATION_READ: 'observation:read',
  OBSERVATION_CONFIG_READ: 'observation:config:read',
  OBSERVATION_CONFIG_MANAGE: 'observation:config:manage',
  ENCOUNTER_CLINICAL_READ: 'encounter:clinical:read',
  ENCOUNTER_CLINICAL_WRITE: 'encounter:clinical:write',
  ENCOUNTER_COMPLETE: 'encounter:complete',
  DIAGNOSIS_WRITE: 'diagnosis:write',
  TREATMENT_WRITE: 'treatment:write',
  NOTE_WRITE: 'note:write',
  TIMELINE_READ: 'timeline:read',
  TREATMENT_EPISODE_READ: 'treatment_episode:read',
  TREATMENT_EPISODE_WRITE: 'treatment_episode:write',
  REPORT_GENERATE: 'report:generate',
  PRESCRIPTION_READ: 'prescription:read',
  PRESCRIPTION_WRITE: 'prescription:write',
  ALLERGY_READ: 'allergy:read',
  DOCUMENT_READ: 'document:read',
  DOCUMENT_READ_RESTRICTED: 'document:read:restricted',
  DOCUMENT_WRITE: 'document:write',
  DOCUMENT_MANAGE: 'document:manage',
  ALLERGY_WRITE: 'allergy:write',
  SAFETY_OVERRIDE: 'safety:override',
  FOLLOWUP_READ: 'followup:read',
  REFERRAL_READ: 'referral:read',
  REFERRAL_CREATE: 'referral:create',
  REFERRAL_MANAGE: 'referral:manage',
  REFERRAL_COMPLETE: 'referral:complete',
  PROCEDURE_READ: 'procedure:read',
  PROCEDURE_WRITE: 'procedure:write',
  CARE_PLAN_READ: 'care_plan:read',
  CARE_PLAN_WRITE: 'care_plan:write',
  CARE_PLAN_PROGRESS: 'care_plan:progress',
  FOLLOWUP_WRITE: 'followup:write',
  FOLLOWUP_CLOSE: 'followup:close',
  FOLLOWUP_DETECT: 'followup:detect',
} as const;

export const clinicalPermissions: WorkstreamPermissions = {
  permissions: ClinicalPermission,
  descriptions: {
    [ClinicalPermission.PATIENT_REGISTER]: 'Register a new patient',
    [ClinicalPermission.PATIENT_READ]: 'View a patient record',
    [ClinicalPermission.PATIENT_SEARCH]: 'Search patients',
    [ClinicalPermission.PATIENT_UPDATE]: 'Update patient demographics and lifecycle status',
    [ClinicalPermission.PATIENT_MERGE]: 'Merge a duplicate patient record into the surviving one',
    [ClinicalPermission.PATIENT_CONTACT_READ]: 'View patient emergency contacts',
    [ClinicalPermission.PATIENT_CONTACT_WRITE]: 'Record patient emergency contacts',
    [ClinicalPermission.PATIENT_IDENTIFIER_READ]: 'View patient external identifiers',
    [ClinicalPermission.PATIENT_IDENTIFIER_WRITE]: 'Record patient external identifiers',
    [ClinicalPermission.QR_ISSUE]: 'Issue a patient QR identity token',
    [ClinicalPermission.QR_RESOLVE]: 'Resolve a QR token to a patient',
    [ClinicalPermission.ENCOUNTER_CHECKIN]: 'Check a patient in (start an encounter)',
    [ClinicalPermission.ENCOUNTER_READ]: 'View encounters',
    [ClinicalPermission.ENCOUNTER_STATUS]: 'Advance an encounter through the visit workflow',
    [ClinicalPermission.QUEUE_READ]: 'View the reception/clinical queue',
    [ClinicalPermission.APPOINTMENT_READ]: 'View the appointment schedule',
    [ClinicalPermission.APPOINTMENT_SCHEDULE]: 'Book an appointment',
    [ClinicalPermission.APPOINTMENT_UPDATE]: 'Reschedule or modify an appointment',
    [ClinicalPermission.APPOINTMENT_CANCEL]: 'Cancel an appointment',
    [ClinicalPermission.APPOINTMENT_ARRIVAL]:
      'Record arrival, waiting, no-show or left-without-being-seen',
    [ClinicalPermission.APPOINTMENT_OVERBOOK]:
      "Book over a practitioner's existing appointment",
    [ClinicalPermission.SCHEDULE_CONFIG_READ]: 'View appointment types and bookable resources',
    [ClinicalPermission.SCHEDULE_CONFIG_MANAGE]: 'Manage appointment types and bookable resources',
    [ClinicalPermission.INTAKE_RECORD]: 'Record or revise clinical intake (complaint + history)',
    [ClinicalPermission.INTAKE_READ]: 'View clinical intake',
    [ClinicalPermission.VITALS_RECORD]: 'Record a set of vital signs',
    [ClinicalPermission.VITALS_READ]: 'View recorded vital signs',
    [ClinicalPermission.OBSERVATION_RECORD]: 'Record a structured clinical observation',
    [ClinicalPermission.OBSERVATION_READ]: 'View recorded observations',
    [ClinicalPermission.OBSERVATION_CONFIG_READ]: 'View the observation definition catalog',
    [ClinicalPermission.OBSERVATION_CONFIG_MANAGE]: 'Manage observation definitions',
    [ClinicalPermission.ENCOUNTER_CLINICAL_READ]: 'View the clinical record of an encounter',
    [ClinicalPermission.ENCOUNTER_CLINICAL_WRITE]:
      'Open a consultation and write complaint, examination and assessment',
    [ClinicalPermission.ENCOUNTER_COMPLETE]: 'Close a consultation',
    [ClinicalPermission.DIAGNOSIS_WRITE]: 'Record or revise a diagnosis',
    [ClinicalPermission.TREATMENT_WRITE]: 'Record or revise a treatment plan',
    [ClinicalPermission.NOTE_WRITE]: 'Append a clinical note',
    [ClinicalPermission.TIMELINE_READ]: "View a patient's longitudinal clinical timeline",
    [ClinicalPermission.TREATMENT_EPISODE_READ]: 'View treatment episodes and their responses',
    [ClinicalPermission.TREATMENT_EPISODE_WRITE]:
      'Start, stop or record the response to a treatment episode',
    [ClinicalPermission.REPORT_GENERATE]: 'Generate a clinical report (patient or encounter)',
    [ClinicalPermission.PRESCRIPTION_READ]: 'View prescriptions',
    [ClinicalPermission.PRESCRIPTION_WRITE]: 'Issue or cancel a prescription',
    [ClinicalPermission.ALLERGY_READ]: 'View patient allergies',
    [ClinicalPermission.DOCUMENT_READ]: 'View clinical documents',
    [ClinicalPermission.DOCUMENT_READ_RESTRICTED]: 'View restricted-confidentiality documents',
    [ClinicalPermission.DOCUMENT_WRITE]: 'Register a clinical document',
    [ClinicalPermission.DOCUMENT_MANAGE]: 'Supersede or void a clinical document',
    [ClinicalPermission.ALLERGY_WRITE]: 'Record or amend a patient allergy',
    [ClinicalPermission.SAFETY_OVERRIDE]: 'Override a clinical safety alert when prescribing',
    [ClinicalPermission.FOLLOWUP_READ]: 'View the follow-up / recall worklist',
    [ClinicalPermission.REFERRAL_READ]: 'View referrals',
    [ClinicalPermission.REFERRAL_CREATE]: 'Create and order a clinical referral',
    [ClinicalPermission.REFERRAL_MANAGE]: 'Advance the administrative referral lifecycle',
    [ClinicalPermission.REFERRAL_COMPLETE]: 'Record a referral as clinically completed',
    [ClinicalPermission.PROCEDURE_READ]: 'View clinical procedures',
    [ClinicalPermission.PROCEDURE_WRITE]: 'Document, complete or void a clinical procedure',
    [ClinicalPermission.CARE_PLAN_READ]: 'View care plans, goals and interventions',
    [ClinicalPermission.CARE_PLAN_WRITE]: 'Author a care plan, its goals and interventions',
    [ClinicalPermission.CARE_PLAN_PROGRESS]: 'Record progress on care-plan goals and interventions',
    [ClinicalPermission.FOLLOWUP_WRITE]: 'Schedule a follow-up',
    [ClinicalPermission.FOLLOWUP_CLOSE]: 'Complete or cancel a scheduled follow-up',
    [ClinicalPermission.FOLLOWUP_DETECT]: 'Run the follow-up due/overdue detection sweep',
  },
  roleGrants: {
    [RoleKey.RECEPTION]: [
      ClinicalPermission.PATIENT_REGISTER,
      ClinicalPermission.PATIENT_READ,
      ClinicalPermission.PATIENT_SEARCH,
      ClinicalPermission.PATIENT_UPDATE,
      ClinicalPermission.PATIENT_CONTACT_READ,
      ClinicalPermission.PATIENT_CONTACT_WRITE,
      ClinicalPermission.PATIENT_IDENTIFIER_READ,
      ClinicalPermission.PATIENT_IDENTIFIER_WRITE,
      ClinicalPermission.QR_ISSUE,
      ClinicalPermission.QR_RESOLVE,
      ClinicalPermission.ENCOUNTER_CHECKIN,
      ClinicalPermission.ENCOUNTER_READ,
      ClinicalPermission.ENCOUNTER_STATUS,
      ClinicalPermission.QUEUE_READ,
      ClinicalPermission.FOLLOWUP_READ,
      ClinicalPermission.FOLLOWUP_CLOSE,
      ClinicalPermission.APPOINTMENT_READ,
      ClinicalPermission.APPOINTMENT_SCHEDULE,
      ClinicalPermission.APPOINTMENT_UPDATE,
      ClinicalPermission.APPOINTMENT_CANCEL,
      ClinicalPermission.APPOINTMENT_ARRIVAL,
      ClinicalPermission.APPOINTMENT_OVERBOOK,
      ClinicalPermission.SCHEDULE_CONFIG_READ,
      ClinicalPermission.ALLERGY_READ,
      ClinicalPermission.DOCUMENT_READ,
      ClinicalPermission.DOCUMENT_WRITE,
      ClinicalPermission.REFERRAL_READ,
      ClinicalPermission.REFERRAL_MANAGE,
    ],
    [RoleKey.NURSE]: [
      ClinicalPermission.PATIENT_READ,
      ClinicalPermission.PATIENT_SEARCH,
      ClinicalPermission.PATIENT_CONTACT_READ,
      ClinicalPermission.PATIENT_IDENTIFIER_READ,
      ClinicalPermission.QR_RESOLVE,
      ClinicalPermission.ENCOUNTER_READ,
      ClinicalPermission.ENCOUNTER_STATUS,
      ClinicalPermission.QUEUE_READ,
      ClinicalPermission.INTAKE_RECORD,
      ClinicalPermission.INTAKE_READ,
      ClinicalPermission.VITALS_RECORD,
      ClinicalPermission.VITALS_READ,
      ClinicalPermission.ENCOUNTER_CLINICAL_READ,
      ClinicalPermission.TIMELINE_READ,
      ClinicalPermission.TREATMENT_EPISODE_READ,
      ClinicalPermission.PRESCRIPTION_READ,
      ClinicalPermission.FOLLOWUP_READ,
      ClinicalPermission.FOLLOWUP_CLOSE,
      ClinicalPermission.APPOINTMENT_READ,
      ClinicalPermission.APPOINTMENT_ARRIVAL,
      ClinicalPermission.SCHEDULE_CONFIG_READ,
      ClinicalPermission.OBSERVATION_RECORD,
      ClinicalPermission.OBSERVATION_READ,
      ClinicalPermission.OBSERVATION_CONFIG_READ,
      ClinicalPermission.ALLERGY_READ,
      ClinicalPermission.ALLERGY_WRITE,
      ClinicalPermission.DOCUMENT_READ,
      ClinicalPermission.DOCUMENT_READ_RESTRICTED,
      ClinicalPermission.DOCUMENT_WRITE,
      ClinicalPermission.REFERRAL_READ,
      ClinicalPermission.PROCEDURE_READ,
      ClinicalPermission.CARE_PLAN_READ,
      ClinicalPermission.CARE_PLAN_PROGRESS,
    ],
    [RoleKey.DOCTOR]: [
      ClinicalPermission.PATIENT_READ,
      ClinicalPermission.PATIENT_SEARCH,
      ClinicalPermission.PATIENT_CONTACT_READ,
      ClinicalPermission.PATIENT_IDENTIFIER_READ,
      ClinicalPermission.ENCOUNTER_READ,
      ClinicalPermission.ENCOUNTER_STATUS,
      ClinicalPermission.QUEUE_READ,
      ClinicalPermission.INTAKE_RECORD,
      ClinicalPermission.INTAKE_READ,
      ClinicalPermission.VITALS_RECORD,
      ClinicalPermission.VITALS_READ,
      ClinicalPermission.ENCOUNTER_CLINICAL_READ,
      ClinicalPermission.ENCOUNTER_CLINICAL_WRITE,
      ClinicalPermission.ENCOUNTER_COMPLETE,
      ClinicalPermission.DIAGNOSIS_WRITE,
      ClinicalPermission.TREATMENT_WRITE,
      ClinicalPermission.NOTE_WRITE,
      ClinicalPermission.TIMELINE_READ,
      ClinicalPermission.TREATMENT_EPISODE_READ,
      ClinicalPermission.TREATMENT_EPISODE_WRITE,
      ClinicalPermission.REPORT_GENERATE,
      ClinicalPermission.PRESCRIPTION_READ,
      ClinicalPermission.PRESCRIPTION_WRITE,
      ClinicalPermission.FOLLOWUP_READ,
      ClinicalPermission.FOLLOWUP_WRITE,
      ClinicalPermission.FOLLOWUP_CLOSE,
      ClinicalPermission.APPOINTMENT_READ,
      ClinicalPermission.APPOINTMENT_SCHEDULE,
      ClinicalPermission.APPOINTMENT_UPDATE,
      ClinicalPermission.APPOINTMENT_CANCEL,
      ClinicalPermission.APPOINTMENT_OVERBOOK,
      ClinicalPermission.SCHEDULE_CONFIG_READ,
      ClinicalPermission.OBSERVATION_RECORD,
      ClinicalPermission.OBSERVATION_READ,
      ClinicalPermission.OBSERVATION_CONFIG_READ,
      ClinicalPermission.ALLERGY_READ,
      ClinicalPermission.ALLERGY_WRITE,
      ClinicalPermission.SAFETY_OVERRIDE,
      ClinicalPermission.DOCUMENT_READ,
      ClinicalPermission.DOCUMENT_READ_RESTRICTED,
      ClinicalPermission.DOCUMENT_WRITE,
      ClinicalPermission.DOCUMENT_MANAGE,
      ClinicalPermission.REFERRAL_READ,
      ClinicalPermission.REFERRAL_CREATE,
      ClinicalPermission.REFERRAL_MANAGE,
      ClinicalPermission.REFERRAL_COMPLETE,
      ClinicalPermission.FOLLOWUP_DETECT,
      ClinicalPermission.PROCEDURE_READ,
      ClinicalPermission.PROCEDURE_WRITE,
      ClinicalPermission.CARE_PLAN_READ,
      ClinicalPermission.CARE_PLAN_WRITE,
      ClinicalPermission.CARE_PLAN_PROGRESS,
    ],
  },
};
