...
export interface Handler { match: (url: string, method: string) => boolean; status?: number; body: unknown }
...
export function mockApi(permissions: string[], handlers: Handler[] = []): void {
  ...
  vi.stubGlobal(
    ...
    vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return Promise.resolve(new Response(JSON.stringify({ error: { code: 'not_found', message: 'not found' } }), { status: 404, headers: { 'content-type': 'application/json' } }));
    }),
  );
}
...
export const product = (over: Record<string, unknown> = {}): Record<string, unknown> => ({
  id: 'p1', sku: 'SKU1', name: 'Test Product', category: 'supplies', unitOfMeasure: 'unit',
  medicationProductId: null, isBatchTracked: false, isExpiryTracked: false, blockExpiredIssue: true,
  allowNegativeStock: false, reorderThreshold: null, reorderQuantity: null, isBillable: false,
  billingCode: null, unitPrice: null, unitCost: null, isActive: true,
