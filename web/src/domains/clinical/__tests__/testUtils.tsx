...
// Clinical strings are registered globally with the platform i18n registry; do
// it once so `t('clinical.*')` resolves in every test render.
registerClinicalMessages();

/** A route from METHOD+path (as the api client builds it, `/api/...`) to a JSON body. */
...
/**
 * Install a `fetch` mock that always answers `/auth/me` with a seeded principal
 * (so `useAuth` resolves), then routes other calls through `handlers`. Any
 * unmatched call resolves 404 so a test never hangs on a real network.
 */
...
/** Render a domain UI inside the platform providers with a resolved session. */
